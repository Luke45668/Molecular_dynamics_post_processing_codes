...
realisation_name = "log.testout_no806324_wall_pure_output_no_rescale_541709_2_60835_23.0_0.005071624521210362_10_10000_10000_600010_T_1_lbda_0.05071624521210362_gdot_0.002" # with shear 
...
dump_realisation_name= "testout_541709_2_60835_23.0_0.005071624521210362_10_gdot_0.002_10000_600010_equilibrium_test.dump"
...
dump_file_shaped=np.reshape(dump_file_unsorted,(loop_size,number_of_particles_per_dump,columns))

#%% sorting rows 

dump_file_sorted=np.zeros((loop_size,number_of_particles_per_dump,columns))
...
#%%
delta_mom_pos_tensor_from_dump=np.zeros((dump_file_sorted.shape[0]-1,no_SRD,4))




for i in range(0,loop_size-1):
    delta_mom_pos_tensor_from_dump[i,:,0]=(dump_file_sorted[i+1,:,4]- dump_file_sorted[i,:,4]) *  dump_file_sorted[i,:,1]#xx
    delta_mom_pos_tensor_from_dump[i,:,1]= (dump_file_sorted[i+1,:,5]- dump_file_sorted[i,:,5]) *  dump_file_sorted[i,:,2]#yy
    delta_mom_pos_tensor_from_dump[i,:,2]= (dump_file_sorted[i+1,:,6]- dump_file_sorted[i,:,6]) * dump_file_sorted[i,:,3] #zz 
    delta_mom_pos_tensor_from_dump[i,:,3]= (dump_file_sorted[i+1,:,4]- dump_file_sorted[i,:,4]) *  dump_file_sorted[i,:,3] #xz
...
delta_mom_pos_tensor_from_dump_mean=np.mean(np.sum( delta_mom_pos_tensor_from_dump,axis=1)/(delta_t_srd*box_vol),axis=0)/no_SRD

delta_mom_pos_tensor_from_dump=np.sum( delta_mom_pos_tensor_from_dump,axis=1)/(delta_t_srd*box_vol)



#%%
shear_rate_term_from_dump=(erate*delta_t_srd/2)*kinetic_energy_tensor[2:,2]
stress_tensor_xz_from_dump= kinetic_energy_tensor[2:,3]+ shear_rate_term_from_dump+ delta_mom_pos_tensor_from_dump[1:,3]
...
stress_tensor_xx_from_dump=( kinetic_energy_tensor[2:,0] +delta_mom_pos_tensor_from_dump[1:,0])
stress_tensor_yy_from_dump=( kinetic_energy_tensor[2:,1] +delta_mom_pos_tensor_from_dump[1:,1])
stress_tensor_zz_from_dump=( kinetic_energy_tensor[2:,2] +delta_mom_pos_tensor_from_dump[1:,2])
