...
r_particle =10e-6 # for some solutions, rememebrr to check if its 25 or 10
phi=0.0005
N=2
    ...
    for z in range(0,number_of_solutions): 
        ...
        for m in range(org_var_1_index_start,org_var_1_index_end):
             for k in range(org_var_2_index_start,org_var_2_index_end):
                ...
                plt.xlabel('$N_{t}[-]$')
                plt.ylabel('$\dot{\gamma}[1/\\tau]$',rotation='horizontal',labelpad=12)
...
for z in range(0,number_of_solutions): 
     ...
     #for m in range(3,4):    
        #for k in range(org_var_2_index_start,org_var_2_index_end):
            ...
            #plt.yscale('log')
            plt.ylabel(' $d \dot{\gamma}/d t\ [1/\\tau^{2}]$',rotation=0,labelpad=40)
            plt.xlabel('$f_{v,x}[-]$')
            #plt.xlabel('$K[\\frac{\\tau}{\mu}]$')
            plt.xscale('log')
...
for k in range(0,org_var_1.size):  
            ...
            ax1.set_ylabel('$P_{x} \ [\\frac{\mu\ell}{\\tau}]$',rotation=0,labelpad=labelpady )
           ...
           # ax2.legend(frameon=False,loc=0,bbox_to_anchor=(legend_x_pos, legend_y_pos)) 
            ...
            ax3.set_ylabel('$P_{x} \ [\\frac{\mu\ell}{\\tau}]$',rotation=0,labelpad=labelpady )
            ...
            ax4.legend(frameon=False,loc=0,bbox_to_anchor=(1,2.8))
...
def plotting_flux_vs_shear_rate(shear_rate_mean_error_of_both_cells,func4,labelpadx,labelpady,params,fontsize,box_side_length_scaled,number_of_solutions,flux_ready_for_plotting,org_var_1_index,shear_rate_mean_of_both_cells):
    ...
    for z in range(0,number_of_solutions):
        ...
        #for i in range(0,1):
                ...
                #plt.xscale('log')
                plt.xlabel('log($\dot{\gamma}\ [1/\\tau]$)', labelpad=labelpadx,fontsize=fontsize)
                #plt.yscale('log')
...
for k in range(0,org_var_1.size):  
           ...
           # ax2.legend(frameon=False,loc=0,bbox_to_anchor=(legend_x_pos, legend_y_pos)) 
            ...
            ax3.set_ylabel('$v_{x}\ [\\frac{\\tau}{\ell}]$',rotation=0,labelpad=labelpady )
