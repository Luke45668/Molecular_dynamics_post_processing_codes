...
filepath="/Users/luke_dev/Documents/simulation_test_folder/uef_dump_test/"
...
thermal_damp_array=np.array([50,250,500,750,1000])
thermal_damp=1500#thermal_damp_array[4]
log_general_name_string=("log.*_100_"+str(thermal_damp)+"_*K_"+str(K))

force_dump_general_name_string='*_dump_**_100_'+str(thermal_damp)+'_*K_'+str(K)+'.dump'
posvel_dump_general_name_string="*_tensor_*_100_"+str(thermal_damp)+"_*K_"+str(K)+".dump"

cfg_general_name_before_string='before*.cfg'
cfg_general_name_after_string='after*.cfg'
...
#print(len(realisation_name_before_sorted_final_cfg))
...
#%% testing box coords
list_unclean=dump2numpy_box_coords_1tstep( Path_2_dump,realisation_name_posvel_dump_sorted_final[0],200)
list_box_vec=[]
box_vec_array_dump_unsort=np.zeros((100,3,3))
box_vec_array_dump=np.zeros((100,3,3))
for i in range(len(list_unclean)):
    list_box_vec.append(list_unclean[i][5:8])
    for j in range(3):
         split_1=list_box_vec[i][j].split(" ")
         box_vec_array_dump_unsort[i,j,:]=split_1
    xy=box_vec_array_dump_unsort[i,0,2]
    xz=box_vec_array_dump_unsort[i,1,2]
    yz=box_vec_array_dump_unsort[i,2,2]
    xlo=box_vec_array_dump_unsort[i,0,0]-np.min(np.array([0,xy,xz,xy+xz]))
    xhi=box_vec_array_dump_unsort[i,0,1]-np.max(np.array([0,xy,xz,xy+xz]))
    ylo=box_vec_array_dump_unsort[i,1,0]-np.min(np.array([0,yz]))
    yhi=box_vec_array_dump_unsort[i,1,1]-np.max(np.array([0,yz]))
    zlo=box_vec_array_dump_unsort[i,2,0]
    zhi=box_vec_array_dump_unsort[i,2,1]
    box_vec_array_dump[i]=np.array([[xhi-xlo,xy,xz],
                                           [0,yhi-ylo,yz],
                                           [0,0,zhi-zlo]])
    ...
    # offdiagonals= box_vec_array_dump_unsort[i,:,2]
    # diagonal=np.diag( box_vec_array_dump_unsort[i,:,1]-box_vec_array_dump_unsort[i,:,0])
    # box_vec_array_dump[i,0,1]=offdiagonals[0]
    # box_vec_array_dump[i,0,2]=offdiagonals[1]
    # box_vec_array_dump[i,1,2]=offdiagonals[2]
    # box_vec_array_dump[i]=box_vec_array_dump[i]+diagonal

# note this doesnt work as sometime the basis vectors are not aligned with the coordinate axes
...
full_dump_coord_before=np.zeros((100,200,4))
full_dump_coord_after=np.zeros((100,200,4))
velocity_array=np.zeros((100,200,3))
box_vector_array_cfg=np.zeros((100,3,3))
for i in range(100):

    full_dump_coord_before[i],box_vector_array_cfg[i]=cfg2numpy_coords(Path_2_dump,realisation_name_before_sorted_final_cfg[i],
                      n_plates*2, 4)
    sort_array=full_dump_coord_before[i]
    full_dump_coord_before[i] = sort_array[sort_array[:,3].argsort()]
    full_dump_coord_after[i],box_vector_array_cfg[i]=cfg2numpy_coords(Path_2_dump,realisation_name_after_sorted_final_cfg[i],
                      n_plates*2, 4)
...
#         zs=full_dump_coord_after[i][j,2]
#         u=velocity_array[i][j,0]
...
# # for i in range(100):
# #     xs=velocity_array[i][0,0]
# #     ys=velocity_array[i][0,1]
# #     zs=velocity_array[i][0,2]
# #     ax.scatter(xs, ys, zs)

#%% inverting and comparing 
from numpy.linalg import inv as matinv
# rearra
inv_box_vec_array=matinv(box_vec_array_dump[99]) # invert the lammps frame 
Lab_frame=np.identity(3)

posvel_from_dump=dump2numpy_f(dump_start_line_posvel,
                                    Path_2_dump,
                                    realisation_name_posvel_dump_sorted_final[0],
                                    n_plates*2).astype("float")[-200:]
posvel_from_dump_sorted= posvel_from_dump[posvel_from_dump[:,0].argsort()]
# dump file is sorted. cfg is 

coords_from_posvel_dump=posvel_from_dump_sorted[:,2:5]


coords_from_cfg_dump=full_dump_coord_after[99,:,:3]
unscaled_cfg=np.zeros((200,3))
transform_dump_coords=np.zeros((200,3))
Q_matrix=np.matmul(box_vector_array_cfg[99].T,inv_box_vec_array) 
Q_check=np.matmul(Q_matrix,box_vec_array_dump[99])

# A=box_vector_array_cfg[99][0,:]
# B=box_vector_array_cfg[99][1,:]
# C=box_vector_array_cfg[99][2,:]
# volume_box=np.sqrt(np.sum(np.dot(np.cross(A,B),C)**2))
# cell_origin=np.array([-63.6805,-47.1703,-77.2825])
for i in range(200):

    
    #conversion_from_scaled_to_unscaled_mat=np.array([A,,np.cross(A,B)])
    unscaled_cfg[i,:]=np.matmul(box_vector_array_cfg[99][:,:].T,coords_from_cfg_dump[i])#-cell_origin
    
   

   
    transform_dump_coords[i,:]=np.matmul(Q_matrix,coords_from_posvel_dump[i,:])
    print("unscaled_cfg",unscaled_cfg[i])
   # print("dump_coords_init",coords_from_posvel_dump[i])
    print("dump transformed",transform_dump_coords[i])
    print(unscaled_cfg-transform_dump_coords)

comparison=np.abs(unscaled_cfg-transform_dump_coords)
if np.any(np.abs(unscaled_cfg-transform_dump_coords)>1e-3):
     print("comparison incorrect")

plt.scatter(unscaled_cfg[:,0],transform_dump_coords[:,0])
plt.show()


def q_matrix_transform(box_vec_array_cfg,box_vec_array_posvel_dump,
                       coords_from_cfg_dump,coords_from_posvel_dump,n_particles,db_forces,db_dirns):
    inv_box_vec_array=matinv(box_vec_array_posvel_dump) 
    ...
    transform_force_dump=np.zeros((n_particles,6))


    for i in range(n_particles):

        unscaled_cfg[i]=np.matmul(box_vector_array_cfg.T,coords_from_cfg_dump[i])
        transform_dump_coords[i]=np.matmul(Q_matrix,coords_from_posvel_dump[i])
        transform_force_dump[i,0:3]=np.matmul(Q_matrix,db_forces)
        transform_force_dump[i,3:6]=np.matmul(Q_matrix,db_dirns)
    comparison=np.abs(unscaled_cfg-transform_dump_coords)
