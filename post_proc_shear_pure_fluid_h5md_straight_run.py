...
realisation_name_h5_after_sorted=['0']*count_h5_after
realisation_name_h5_before_sorted=['0']*count_h5_after
for i in range(count_h5_after):
     ...
     if data_set==0:
        ...
        realisation_name_h5_after_sorted[(count_h5_after-(2*j_))+realisation_index_]=realisation_name_h5_after[i]
     else:
        realisation_name_h5_after_sorted[(count_h5_after-j_)+realisation_index_]=realisation_name_h5_after[i]
for i in range(count_h5_after):
     ...
     if data_set==0:
        ...
        realisation_name_h5_before_sorted[(count_h5_after-(2*j_))+realisation_index_]=realisation_name_h5_before[i]
     else:
        realisation_name_h5_before_sorted[(count_h5_after-j_)+realisation_index_]=realisation_name_h5_before[i]
...
def stress_tensor_total_compute_shear(terms_9,terms_6,box_vol,realisation_index,delta_mom_pos_tensor_summed_shared,stress_tensor_summed_shared,kinetic_energy_tensor_summed_shared,realisation_name_h5_after,realisation_name_h5_before,shape_after,j_,no_data_sets,erate,delta_t_srd,p):    
    ...
    with h5.File(realisation_name_h5_after, 'r') as f_a:
        with h5.File(realisation_name_h5_before, 'r') as f_b:
            ...
            count=(shape_after[0]-1)*p # to make sure the data is in the correct location in 1-D arrat 
...
if __name__ =='__main__':
        ...
        stress_tensor_summed=np.reshape(stress_tensor_summed_shared,((3,3,shape_after[0]-1,9)))
        kinetic_energy_tensor_summed=np.reshape(kinetic_energy_tensor_summed_shared,((3,3,shape_after[0]-1,6)))
        delta_mom_pos_tensor_summed=np.reshape(delta_mom_pos_tensor_summed_shared,((3,3,shape_after[0]-1,9)))
        ...
        # saving results 
        np.save("stress_tensor_summed_test_M_"+str(rho)+"_L_"+str(box_size),stress_tensor_summed)
        np.save("delta_mom_pos_tensor_summed_test_M_"+str(rho)+"_L_"+str(box_size),delta_mom_pos_tensor_summed)
        np.save("kinetic_energy_tensor_summed_test_M_"+str(rho)+"_L_"+str(box_size),kinetic_energy_tensor_summed)
