...
filepath='KATHLEEN_LAMMPS_RUNS/flat_elastic_tests'
...
def log_error_detector(filepath,var_choice_1,var_choice_2,realisation_name_log, count_log):
    ...
    general_error_message=bytes("ERROR",'utf-8')


    for i in range(0,count_log):
        split_name_for_org =  realisation_name_log[i].split('_')
        var_choice_1_posn= split_name_for_org[17] # need 
        var_choice_2_posn= split_name_for_org[10]
        realisation_posn= split_name_for_org[6]
        var_choice_1_index= np.where(var_choice_1_posn==var_choice_1)
        var_choice_2_index= np.where(var_choice_2_posn==var_choice_2)
        realisation_index_= np.where(realisation_posn==realisation_index)
        with open(realisation_name_log[i]) as f:
            ...
            if read_data.find(general_error_message) != -1:
                ...
                error_matrix[realisation_index_,var_choice_1_index,var_choice_2_index]=1


            else:
                print('Successful run')
                error_matrix[var_choice_1_index,var_choice_2_index,realisation_index_]=0
